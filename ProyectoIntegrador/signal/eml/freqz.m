...
if all(size(b)>[1 1])
  ...
  eml_lib_assert(size(b,2) == 6 && isa(b,'double'),'signal:freqz:invalidinputsosmatrix',...
    'When first input is a matrix, it must be double precision and have exactly 6 columns to be a valid SOS matrix. Otherwise, the first input must be a double precision vector.');
...
if isTF   
  ...
  eml_lib_assert(isa(aDen,'double') && isa(b,'double'),...
    'signal:freqz:inputNotDouble',...
...
if fvflag
  ...
  w = freqz_freqvec(nfft, Fs, sided_switch).';
