...
if this.IsSOSConvertedMCode
  ...
  tmpCode = [];
  if ~isempty(strfind(codeStr,'fdesign'))
  ...
  mcode.add(['  % ' FilterDesignDialog.message('CCodegenCommandLineHelp') sprintf('\n')]);
  mcode.addcr;
...
elseif ~isempty(strfind(codeStr,'fdesign'))
  ...
  try
    ...
    mcode.insert(1,['  % ' FilterDesignDialog.message('CCodegenCommandLineHelp') sprintf('\n')]);
    mcode.addcr;
