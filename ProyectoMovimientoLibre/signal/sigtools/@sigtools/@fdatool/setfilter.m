...
if isfield(options, 'mcode') && ~isempty(options.mcode),
  ...
  if isempty(this.BaseMCode), this.BaseMCode = sigcodegen.mcodebuffer; end
  if isempty(this.MultirateMCode), this.MultirateMCode = sigcodegen.mcodebuffer; end
  if isempty(this.FxPtMCode), this.FxPtMCode = sigcodegen.mcodebuffer; end
