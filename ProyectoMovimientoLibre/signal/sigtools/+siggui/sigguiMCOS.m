classdef (Abstract) sigguiMCOS < dynamicprops & matlab.mixin.SetGet & matlab.mixin.Copyable & matlab.mixin.internal.TreeNode & matlab.mixin.Heterogeneous
  ...
  methods
    ...
    function [label selectedinputprocessing offset] = inputProcessingSelector(this,pos,varargin)
      ...
      if ~isempty(varargin)
        addInheritedSpec = varargin{1};
      else
        addInheritedSpec = true;
      end
      ...
      if addInheritedSpec
        specsX = [specsX  {siggui.message(...
          'signal:siggui:siggui:inputProcessingSelector:Inherited')}];
        specs = [specs  {'Inherited (this choice will be removed - see release notes)'}];
      end
