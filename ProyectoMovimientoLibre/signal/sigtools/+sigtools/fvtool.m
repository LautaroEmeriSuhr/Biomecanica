classdef (CaseInsensitiveProperties=true) fvtool < sigtools.sigfig & sigio.dyproputil & matlab.mixin.Copyable
  ...
  methods
    function set.AnalysisToolbar(obj,value)
      % DataType = 'on/off'
      validatestring(value,{'on','off'},'','AnalysisToolbar');
      obj.AnalysisToolbar = value;
    end
    
    function set.FigureToolbar(obj,value)
      % DataType = 'on/off'
      validatestring(value,{'on','off'},'','FigureToolbar');
    ...
    function set.DesignMask(obj,value)
      % DataType = 'on/off'
      validatestring(value,{'on','off'},'','DesignMask');
    ...
    function set.FsEditable(obj,value)
      % DataType = 'on/off'
      validatestring(value,{'on','off'},'','FsEditable');
    ...
    function [filters, hasfs] = findfilters(this, varargin)
      ...
      for indx = index.object
        ...
        if ~isempty(mi)
          ...
          % filters(indx).Filter.addprop('MaskInfo');
      ...
      for indx = 1:length(filters)
        ...
        if ~isempty(hspecs) && ~hspecs.NormalizedFrequency
          ...
          hasfs = true;
        elseif isprop(filters(indx).Filter,'SysObjParams') && isfield(filters(indx).Filter.SysObjParams,'SampleRate')
          set(filters(indx), 'Fs', filters(indx).Filter.SysObjParams.SampleRate);
          hasfs = true;
...
hmenus.hedit(end+1) = hmenus.htools(2);
set(hmenus.hedit(end), 'Parent', hmenus.hedit(1), 'Position', 1);
hmenus.htools(2) = [];
