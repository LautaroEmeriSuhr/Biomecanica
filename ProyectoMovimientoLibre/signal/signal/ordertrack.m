...
%
%   [...] = ORDERTRACK(X,Fs,RPM,ORDERLIST,RPMREFIDX,...,'Decouple',DC)
%   extracts order magnitudes simultaneously when DC is TRUE or
%   individually when DC is FALSE. Order magnitudes extracted
...
%   reduce memory requirements and computation time. This option is
%   recommended for large input signals. When a segment length, SL, is
%   provided, ORDERTRACK computes the order magnitude for each segment and
...
%   If SL is not specified, the entire input signal, X, is processed in one
%   step.
%
...
if isscalar(varargin{2})
  ...
  fs = varargin{2};
  rpm = varargin{3};
  orderlist = varargin{4};
  varargin(1:4) = [];
  
  % Parse name-value pairs 
  [amp,scale,bw,sl,refidx,dc] = parseinputs(x,fs,rpm,orderlist,varargin{:});
  ...
  x = x(:);
  rpm = signal.internal.toColIfVect(rpm);
  orderlist = orderlist(:);
  refidx = refidx(:);
  bw = bw(:);
else
  ...
  inputType = 'map';
  if nargin > 5
    error(message('signal:rpmmap:MAPORDERTRACKNOARGS'));
  end
  narginchk(5,5);
  map = varargin{1};
  order = varargin{2};
...
if strcmp(inputType,'map')
  % We have a map. Extract magnitudes.
  mag = computeMAGwithMAP(map,order,orderlist);
  outtime = maptime';
  outrpm = maprpm';
else  
  % We have an input signal and rpm vector. Compute the map if rpm is a
...
% Cast output to match mag
if isa(mag,'single')
  outtime = single(outtime);
  outrpm = single(outrpm);
end
...
%--------------------------------------------------------------------------
function [amp,scale,bw,sl,refidx,dc] = parseinputs(x,fs,rpm,orderlist,varargin) 

if nargin > 4 && ~ischar(varargin{1})
  % Error out if refidx is provided but rpm is a vector
  if isvector(rpm) 
    error(message('signal:rpmmap:RPMMustBeMatrix'));
  end
  refidx = varargin{1};
  varargin(1) = [];
else
  ...
  if ~isvector(rpm)
    error(message('signal:rpmmap:MustbeVectorRPMREFIDX'));
  end  
end
...
% Parse Name-value pairs
p = inputParser;
p.addParameter('Amplitude',amp);
p.addParameter('Scale',scale);
p.addParameter('Bandwidth',bw);
p.addParameter('SegmentLength',sl);
p.addParameter('Decouple',dc);
parse(p,varargin{:});
amp = p.Results.Amplitude;
scale = p.Results.Scale;
bw = p.Results.Bandwidth;
sl = p.Results.SegmentLength;
dc = p.Results.Decouple;

% If bw is a scalar, turn it into a vector
if isscalar(bw) && length(orderlist)>1
  bw = bw*ones(length(orderlist),1);
end
...
% Check that tm,amp,and scale match to allowed values
amp = validatestring(amp,{'peak','rms','power'},'ordertrack','AMP');
scale = validatestring(scale,{'linear','dB'},'ordertrack','SCALE');
%--------------------------------------------------------------------------
function validateInputs(x,fs,rpm,orderlist,bw,sl,refidx,dc,map,order,maprpm,maptime,bMap)
if strcmp(bMap,'signal')
  validateattributes(x,{'single','double'},...
    {'real','nonsparse','finite','nonnan','vector'},'ordertrack','X');
  validateattributes(fs,{'numeric'},...
    {'real','positive','nonsparse','finite','nonnan','scalar'},'ordertrack','Fs');
  validateattributes(rpm,{'numeric'},...
    {'real','positive','nonsparse','finite','nonnan','nonempty'},'ordertrack','RPM');
  validateattributes(orderlist,{'numeric'},...
      {'real','positive','nonsparse','finite','nonnan','vector'},'ordertrack','ORDERLIST');
  validateattributes(bw,{'numeric'},...
    {'real','positive','nonsparse','finite','nonnan','vector'},'ordertrack','BW');
  validateattributes(sl,{'numeric'},...
    {'real','positive','nonsparse','integer','finite','nonnan','scalar',...
  ...
  validateattributes(refidx,{'numeric'},...
    {'real','positive','nonsparse','integer','finite','nonnan','vector',...
  ...
  if ~(isvector(rpm)&&length(rpm)==length(x)||size(rpm,1)==length(x))
    error(message('signal:rpmmap:RPMMustBeVectorMatrix'));
  end
  ...
  validateattributes(map,{'single','double'},...
    {'real','nonnegative','nonsparse','finite','nonnan','nonempty'},'ordertrack','MAP');
  validateattributes(maprpm,{'numeric'},...
    {'real','positive','nonsparse','finite','nonnan','vector'},'ordertrack','RPM');
  ...
  validateattributes(order,{'numeric'},...
    {'real','nonnegative','nonsparse','finite','nonnan','vector'},'ordertrack','ORDER');
  validateattributes(orderlist,{'numeric'},...
    {'real','positive','nonsparse','finite','nonnan','vector',...
    '<=',max(order),'>=',min(order)},'ordertrack','ORDERLIST');
  ...
  % Check that map and orpm have consistent dimensions
  if ~(size(map,2)==length(maprpm))
    error(message('signal:rpmmap:RPMMatchMap'));
  end
end
...
xAmp = zeros(xlen,numOrd,class(x));
F = zeros(size(rpm,1),numOrd);
...
mag = zeros(length(orderlist),size(map,2),class(map));
...
if strcmp(bInput,'signal')
  if isempty(maptime)
    ...
    % Resampling - trucate signaltime to range of maptime
    [~,scale,units]=engunits(signalrpm);
