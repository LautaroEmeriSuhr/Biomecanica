...
[bw,sl,refidx,dc,fo] = parseinputs(x,fs,rpm,orderlist,varargin{:});
...
xrec = zeros(length(x),numOrd,class(x));

% Compute frequency track matrix
F = orderlist'.*rpm(:,refidx)/60;
...
if dc
  % Call the Vold-Kalman filter on coupled orders
  xrec = signal.internal.vk(x,fs,F,r,fo,sl);
else
  % Call the Vold-Kalman filter on uncoupled orders, one by one
  for i = 1:numOrd
    xrec(:,i) = signal.internal.vk(x,fs,F(:,i),...
      r(i),fo,sl);
  end
end
%--------------------------------------------------------------------------
...
validateattributes(x,{'single','double'},...
  {'real','nonsparse','finite','nonnan','vector'},'orderwaveform','X');
validateattributes(fs,{'numeric'},...
  {'real','positive','nonsparse','finite','nonnan','scalar'},'orderwaveform','Fs');
validateattributes(rpm,{'numeric'},...
  {'real','positive','nonsparse','finite','nonnan','nonempty'},'orderwaveform','RPM');
validateattributes(orderlist,{'numeric'},...
  {'real','positive','nonsparse','finite','nonnan','vector'},'orderwaveform','ORDERLIST');
validateattributes(bw,{'numeric'},...
  {'real','positive','nonsparse','finite','nonnan','vector'},'orderwaveform','BW');
...
validateattributes(refidx,{'numeric'},...
  {'real','positive','nonsparse','integer','nonsparse','finite','nonnan','vector',...
  'numel',length(orderlist),'<=',nrpm},'orderwaveform','REFIDX');
validateattributes(dc,{'numeric','logical'},...
  {'nonsparse','scalar','nonnan'},'orderwaveform','DC');
validateattributes(fo,{'numeric'},...
  {'real','positive','nonsparse','integer','finite','nonnan','<=',2,'scalar'},'orderwaveform','FO');
