...
elseif isempty(ProbDet) 
  ...
  if nargout>3
    error(message('signal:plomb:TooManyOutputArgs'));
  end
  varargout{1} = P;
  varargout{2} = fr;
  varargout{3} = z;      
...
if ~isempty(arglist)
  ...
  % DetectionProbability Flag: Validate and cache (default - empty)
...
% Any more arguments remaining are invalid inputs
if numel(arglist)>0  
