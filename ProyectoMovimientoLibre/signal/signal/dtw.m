...
if strcmp(metric,'symmkl')
  if ~isreal(x) || ~isreal(y) || any(x(:)<0) || any(y(:)<0)
    error(message('signal:dtw:MustBeRealPositive'))
  end
end
...
if ~isempty(x) && ~isempty(y)
  if ~isempty(varargin)
    r = varargin{1};
    validateattributes(r,{'numeric'},{'finite','integer','positive','scalar','real'},'dtw','MAXSAMP',3)
    C = constrainedCumulativeDistance(x, y, double(r), metric);
  else
    C = unconstrainedCumulativeDistance(x, y, metric);
  end
...
if isreal(x) && isreal(y)
  ...
  C = dtwmex([real(x); imag(x)], [real(y); imag(y)], metric);
end
...
if m<n
  if isreal(x) && isreal(y)
    C = dtwmex(y, x, r, metric)';
  elseif ~isreal(x) && ~isreal(y)
    C = dtwmex([real(y); imag(y)], [real(x); imag(x)], r, metric)';
  end
elseif isreal(x) && isreal(y)
  C = dtwmex(x, y, r, metric);
else
  C = dtwmex([real(x); imag(x)], [real(y); imag(y)], r, metric);
end
