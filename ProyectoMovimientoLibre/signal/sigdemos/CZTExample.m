function CZTExample(action,s)
%CZTExample Example showing the FFT and CZT in the Signal Processing Toolbox.
%
% This example shows how to compare the transform of a filter using two
% different algorithms. They are
...
labelStr='FFT';
callbackStr='CZTExample(''radio'',1);';
...
yPos=btnTop-(btnNumber-1)*(btnHt+spacing);
labelStr='CZT';
callbackStr='CZTExample(''radio'',2);';
...
% Text field
textPos = [labelRight-textWidth labelBottom textWidth textHeight];
callbackStr = 'CZTExample(''radius1'')';
...
% Text field
textPos = [labelRight-textWidth labelBottom textWidth textHeight];
callbackStr = 'CZTExample(''radius2'')';
...
% The INFO button
labelStr='Info';
callbackStr='CZTExample(''infobtn'')';
helpHndl=uicontrol( ...
	'Style','pushbutton', ...
	'Units','normalized', ...
...
FsHndl = findobj(ud.hndlList,'Tag','Fs'); 
v = get(FsHndl,'UserData');
s = get(FsHndl,'String');

vv = eval(s,num2str(v));
if vv<Wmax*2, vv = v; end
vv = round(vv*100)/100;
set(FsHndl,'UserData',vv,'String',num2str(double(vv)))
design
...
WminHndl = findobj(ud.hndlList,'Tag','Fmin'); 
v = get(WminHndl,'UserData');
s = get(WminHndl,'String');

vv = eval(s,num2str(v));
if vv>=Wmax, vv = v; end
vv = round(vv*100)/100;
set(WminHndl,'UserData',vv,'String',num2str(vv))
design
...
WmaxHndl = findobj(ud.hndlList,'Tag','Fmax'); 
v = get(WmaxHndl,'UserData');
s = get(WmaxHndl,'String');

vv = eval(s,num2str(v));
if vv<=Wmin, vv = v; end
vv = round(vv*100)/100;
set(WmaxHndl,'UserData',vv,'String',num2str(vv))
design

function points

ud = get(gcf,'UserData');
pointsHndl = findobj(ud.hndlList,'Tag','points');

v = get(pointsHndl,'UserData');
s = get(pointsHndl,'String');

vv = eval(s,num2str(v));
if vv<length(ud.filter.den), vv = v; end
vv = round(vv*100)/100;
set(pointsHndl,'UserData',vv,'String',num2str(vv))
design

function radius1

ud = get(gcf,'UserData');
radius1Hndl = findobj(ud.hndlList,'Tag','Radius1');

v = get(radius1Hndl,'UserData');
s = get(radius1Hndl,'String');

vv = eval(s,num2str(v));
if vv<=0 | vv>10, vv = v; end
vv = round(vv*1000)/1000;
set(radius1Hndl,'UserData',vv,'String',num2str(vv))
design

function radius2

ud = get(gcf,'UserData');
radius2Hndl = findobj(ud.hndlList,'Tag','Radius2'); 

v = get(radius2Hndl,'UserData');
s = get(radius2Hndl,'String');

vv = eval(s,num2str(v));
if vv<=0 | vv > 10, vv = v; end
vv = round(vv*1000)/1000;
set(radius2Hndl,'UserData',vv,'String',num2str(vv))
design
